...
class DemandFirstWaypointsUtilization(GenericSR):
    BIG_M = 10 ** 9
    ...
    def __demands_first_waypoints(self):
        ...
        for d_map_idx in range(len(self.__demands)):
            ...
            if (s, t) not in potential_waypoints_dict:
                ...
                # Get the utilization of the neighbors
                    utilizations = [np.sum(best_util_map[neighbor]) for neighbor in neighbors]
                # Combine neighbors and their utilization
                    neighbors_utilizations = list(zip(neighbors, utilizations))
                # Sort by utilization
                    neighbors_utilizations.sort(key=lambda x: x[1])
                # Take the top-k neighbors with lowest utilization
                    top_k = neighbors_utilizations[:top_k_neighbors]
    ...
    def get_name(self):
        ...
        return f"demand_first_waypoints_utilization"