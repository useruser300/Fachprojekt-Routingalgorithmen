...
class DemandFirstWaypointsAvailableCapacities(GenericSR):
    BIG_M = 10 ** 9
    ...
    def __demands_first_waypoints(self):
        ...
        for d_map_idx in range(len(self.__demands)):
            ...
            # Get the available capacities of the neighbors
                    available_capacities = [self.__capacity_map[node][neighbor] - best_flow_map[node][neighbor] for neighbor in neighbors]
            # Combine neighbors and their available capacities
                    neighbors_capacities = list(zip(neighbors, available_capacities))
    ...
    def get_name(self):
        ...
        return f"demand_first_waypoints_available"